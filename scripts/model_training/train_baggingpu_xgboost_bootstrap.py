#!/usr/bin/env python3
"""07: Train BaggingPU(XGBoost) Bootstrap Ensemble"""
...
def train_bootstrap_ensemble(
    ...
    logger.info("=" * 70)
    logger.info(f"BaggingPU(XGBoost) Bootstrap Ensemble - Outer Fold {outer_fold}")
    logger.info("=" * 70)
    ...
    base_estimator_params = structured_params['base_estimator']

    logger.info(f"Loaded best params from Grid Search (PR-AUC: {best_params_data['best_value']:.4f})")
    logger.info(f"  Bagging: n_estimators={bagging_params['n_estimators']}, max_samples={bagging_params['max_samples']:.3f}")
    logger.info(f"  Base estimator: n_estimators={base_estimator_params['n_estimators']}, max_depth={base_estimator_params['max_depth']}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Train BaggingPU(XGBoost) bootstrap ensemble"
    )
    parser.add_argument(
        "--outer-fold",
        type=int,
