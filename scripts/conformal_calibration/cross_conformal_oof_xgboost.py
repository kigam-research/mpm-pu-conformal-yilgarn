#!/usr/bin/env python3
"""Cross-Conformal Calibration for XGBoost - OOF Pooling Method (Alternative 2)"""
...
def run_oof_conformal(
    ...
    """Run OOF Pooling cross-conformal calibration for a specific outer fold."""
    logger.info("=" * 70)
    logger.info(f"XGBoost OOF Pooling Cross-Conformal - Outer Fold {outer_fold}")
    logger.info("=" * 70)
    ...
    logger.info(f"Calibrated OOF: mean={calibrated_oof.mean():.6f}, "
                f"std={calibrated_oof.std():.6f}")
    ...
    logger.info(f"FNR: {fnr_info['fnr']:.4f}")
    logger.info(f"FNR upper: {fnr_info['fnr_upper']:.4f}")
    logger.info(f"Hoeffding: {fnr_info['hoeffding']:.4f}")
    logger.info(f"N positives: {fnr_info['n_pos']}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run OOF Pooling cross-conformal calibration for XGBoost"
    )
    parser.add_argument(
        "--outer-fold",
        type=int,
