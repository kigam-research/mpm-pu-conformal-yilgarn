#!/usr/bin/env python3
"""08: Cross-Conformal Calibration for XGBoost"""
...
def run_cross_conformal(
    ...
    logger.info("=" * 70)
    logger.info(f"XGBoost Cross-Conformal Calibration - Outer Fold {outer_fold}")
    logger.info("=" * 70)
    ...
    if not bootstrap_pred_path.exists():
        ...
        from scripts.utils import PlattApplicator
...
def main():
    parser = argparse.ArgumentParser(
        description="Run cross-conformal calibration for XGBoost"
    )
    parser.add_argument(
        "--outer-fold",
        type=int,
