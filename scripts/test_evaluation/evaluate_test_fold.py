...
def evaluate_test_fold(
    ...
    logger.info("=" * 70)
    logger.info(f"Test Fold Evaluation - {method.upper()} - Outer Fold {outer_fold}")
    logger.info("=" * 70)
    ...
    if not bootstrap_pred_path.exists():
        ...
        logger.error(f"Run the bootstrap training script for {method} first!")
        sys.exit(1)
    ...
    if test_predictions_raw is not None:
        ...
        q25 = None
        q75 = None
        logger.warning("test_predictions not found in bootstrap_predictions.npz, q25/q75 will be None")
    ...
    if not median_params_path.exists():
        logger.error(f"Median params not found: {median_params_path}")
        logger.error("Run cross-conformal calibration script first!")
        sys.exit(1)

    median_params = load_json(median_params_path)
    logger.info(f"Loaded median params:")
    logger.info(f"  Platt coef: {median_params['platt_coef']:.4f}")
    logger.info(f"  Platt intercept: {median_params['platt_intercept']:.4f}")
    logger.info(f"  Threshold: {median_params['threshold']:.4f}")
    ...
    logger.info(f"Set width distribution: 0={sum(set_width == 0)}, "
                f"1={sum(set_width == 1)}, 2={sum(set_width == 2)}")

    logger.info("\nComputing zone classifications...")

    zone_method = config.get('zone_method', 'percentile_std')

    if zone_method == 'prob' and q25 is not None and q75 is not None:
        ...
        logger.info(f"Using rel_IQR 30% zone classification (rel_IQR <= {prob_config.get('rel_iqr_threshold', 0.30):.0%})")
    else:
    ...
    test_fold_data = outer_folds[outer_folds['fold_id'] == outer_fold]
    test_indices_from_folds = test_fold_data['index'].values
    ...
    logger.info("Test fold evaluation complete!")
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate model on outer test fold"
    )
    parser.add_argument(
        "--outer-fold",
        type=int,
    ...
    evaluate_test_fold(args.outer_fold, args.method)
